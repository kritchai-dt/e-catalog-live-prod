...
<html>
 <head>
  ...
  <meta http-equiv="Content-Security-Policy" content="default-src 'self' 'unsafe-inline' 'unsafe-eval' *.cdn.sdelivr.net *.w3.org *.fontawesome.com www.google-analytics.com www.googletagmanager.com  cdn.jsdelivr.net unpkg.com code.jquery.com img.youtube.com " />
  ...
  <div class="section">
    ...
    <div class="container" id="crudApp">
      ...
      <div  class="menu">
        <div class="row">
          <div class="col-lg-7">
            <h2>การจัดการข้อมูลหน้า Video</h2>
          </div>
        ...
        <div class="row">
          <div class="col-6">
            <input type="button" class="btn btn-success btn-xs" data-bs-toggle="modal" data-bs-target="#modalVideo"
              @click="openModel" value="เพิ่มข้อมูล" />
          </div>
        </div>
        <div class="row " >
          <div class="col-6 col-md-4 col-lg-3 mt-4" v-for="row in allData">
            <div class="box-content"  data-bs-toggle="modal" data-bs-target="#modalVideo" @click="fetchData(row.id)">
            <button type="button" name="delete" class="btn btn-danger btn-xs delete" @click="deleteData(row.id)">X</button>
                <img class="img-content video-thumb" :class="{ 'img-disable': row.display_flag == 'N' }" v-bind:src="'https://img.youtube.com/vi/' +  row.code_yt + '/hqdefault.jpg'">
            </div>
          </div>
        </div>
        <div v-if="modalVideo" id="modalVideo" class="modal fade" tabindex="-1">
          <div class="modal-dialog modal-lg">
            <div class="modal-content">
              ...
              <div class="modal-body">
                <div class="row mb-3">
                  <label class="col-sm-3 col-form-label font-20">URL youtube</label>
                  <div class="col-sm-9">
                      <input type="text" class="form-control" v-model="url_yt"  />
                      <br>
                      <h5 class="text-danger"> เลือกวิดีโอใน Youtube กดปุ่ม "แชร์" คัดลอก URL ตัวอย่าง : https://youtu.be/ABC456XXXX</h5>
                  </div>
                </div>

                <div v-if="code_yt != ''" class="row mb-3">
                  <label class="col-sm-3 col-form-label font-20"></label>
                  <div class="col-sm-9">
                    <input type="hidden" v-model="code_yt" />
                    <img  v-bind:src="'https://img.youtube.com/vi/'+ code_yt +'/hqdefault.jpg'" class='img-thumbnail'  width='300'>
                  </div>
                </div>
...
let app = new Vue({
 ...
 methods:{
  ...
  submitData(){
    this.code_yt = this.url_yt.substr(17);
   if(app.url_yt != '' && app.display_flag != '') {
  ...
  fetchData(id){
   axios.post('action.php', {
    action:'fetchSingleVideo',
    id:id
   }).then(res => {
    app.url_yt = res.data.url_yt;
    app.display_flag = res.data.display_flag;
    app.code_yt = res.data.code_yt;
    app.hiddenId = res.data.id;
    app.modalVideo = true;
    app.actionButton = 'Update';
    app.dynamicTitle = 'Edit video';
   });
  },
  deleteData(id){
    modalVideo = false;
